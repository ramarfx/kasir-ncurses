...
void TerminalUI::initWindows() {
  ...
  mainWin =
      newwin(middleHeight, screenWidth - menuWidth, HEADER_HEIGHT, menuWidth);
  contentWin =
      derwin(mainWin, middleHeight - 2, screenWidth - menuWidth - 2, 1, 1);
  statusWin = newwin(STATUS_HEIGHT, screenWidth,
                     screenHeight - INFO_HEIGHT - STATUS_HEIGHT, 0);
...
void TerminalUI::drawBoxWithTitle(WINDOW *win, const std::string &title,
                                  int colorPair) {
  drawRoundedBox(win);
...
void TerminalUI::drawMain() {
  ...
  case Menu::LihatMenu:
    ...
    break;
  case Menu::BuatPesanan:
    currentPage = &createPesananPage;
    break;
  case Menu::LihatAntrian:
    currentPage = &showAntrianPage;
    break;
  case Menu::ProsesAntrian:
    currentPage = &processAntrianPage;
    break;
  default:
    currentPage = &menuPage;
    break;
  ...
  // TAMPILKAN: Panggil fungsi draw milik halaman yang sedang ditunjuk
  if (currentPage != nullptr) {
    currentPage->clearWin(contentWin); // Bersihkan sisa teks halaman sebelumnya
    currentPage->draw(contentWin);     // Gambar halaman baru
  }
...
void TerminalUI::drawStatus() {
  ...
  string currentMsg = TerminalUI::statusMsg.empty() ? "Ready" : statusMsg;
